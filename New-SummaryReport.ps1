function New-SummaryReport {
    ...
    .INPUTS
        System.String. New-SummaryReport accepts string values for SystemScan
        and WebScan parameters.
    .OUTPUTS
        Com.Excel.Application. New-SummaryReport returns an Excel spreadsheet
    .EXAMPLE
        PS C:\> New-SummaryReport -SystemScan $Sys -WebScan $Web
    ...
    if ( $PSBoundParameters.ContainsKey('DatabaseScan') ) {
        ...
        $UDbCsv | ForEach-Object -Process {
            ...
            $_ | Add-Member -MemberType NoteProperty -Name 'TFS' -Value '--'
        }
    }
    ...
    if ( $PSBoundParameters.ContainsKey('SystemScan') ) {
        ...
        foreach ( $object in $USysCsv ) {
            ...
            $object | Add-Member -MemberType NoteProperty -Name 'TFS' -Value '--'
    ...
    else { $Splat.SavePath = Join-Path -Path "$HOME\Desktop" -ChildPath ('Scan-Summary-Report_{0}.xlsx' -f (Get-Date -F "yyyy-MM")) }
