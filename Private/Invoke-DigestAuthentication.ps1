...
function Invoke-DigestAuthentication {
    <# =========================================================================
    .SYNOPSIS
    ...
    Begin {
        if ( $Uri.Scheme -NE 'https' ) { Throw 'Scheme does not match "https". You must use a secure connection' }
    ...
    Process {
        ...
        try {
            ...
            $failResponce = $_.Exception.Response
            if ( !$failResponce ) {
                Write-Error -Message 'Failed to communicate with server'
            }
            else {
                $headerParam = @{ Header = $failResponce.Headers.Where({ $_.Key -eq 'WWW-Authenticate' }).Value }
                ...
                $requestParams = @{
                    Uri     = $Uri
                    Headers = @{
                        Authorization = Get-DigestHeader -Tokens $authParams #-Verbose
                    }
                }
